...
// Normalize a course code by trimming, converting to uppercase, and removing any spaces.
...
async function getCourseCode(
  ...
  // Determine the user query: prefer rawText if provided
  let userQuery;
  if (rawText && rawText.trim()) {
    ...
    try {
      ...
      const transcription = await client.audio.transcriptions.create({
        file: [audioFilePath, audioBuffer],
  ...
  // Build the system prompt using a JSON schema (here, a simplified version).
...
Ensure you strictly follow the JSON schema.`;
  ...
  try {
    ...
    // Filter out codes not in allowedCodes
...
- status, crn, course_code, section, course_title, credits, schedule_type, instructor, day, start_time, end_time, and also_register_in.
...
GENERATION INSTRUCTIONS: Generate multiple course schedules (aim for at least 3) with each schedule having a maximum of 2.5 credits. For courses with tutorials, always select the tutorial that corresponds to the lecture, and only include a tutorial if its parent lecture is in the schedule. You cannot include two tutorials for the same course. Only output schedules that contain every mandatory course.`;
...
// ----------------------------------------------------------------------
// 5) SCRAPER: fetchCoursesForDepartment
// ----------------------------------------------------------------------
async function fetchCoursesForDepartment(department, termCode = "202510") {
  // This string exactly mimics the Python form_data_template.
  ...
  try {
    ...
    // Find all rows with bgcolor="#C0C0C0" or "#DCDCDC"
    ...
    while (i < rowCount) {
      ...
      if (cells.length >= 11) {
        ...
        // Process subsequent rows for this course.
        i++;
        while (i < rowCount) {
          // Get the next row text (using a space as separator, similar to BeautifulSoup’s get_text(separator=" ", strip=True))
...
async function main() {
  ...
  // (A) GET COURSE CODES (prefer raw text if available)
  const audioFilePath = ""; // Empty so raw text is used
  const rawTextInput = "I want to mandatory register COMP 1405, COMP 1805, MATH 1007, BIOL1902, PSYC1001";
  // const rawTextInput = ""
  ...
  const codesResponse = await getCourseCode(
    ...
    "Clarify domain-specific terms if needed.",
    "llama-3.1-8b-instant"
  );

  console.log("\n--- GET COURSE CODES RESPONSE ---");
  console.log(JSON.stringify(codesResponse, null, 2));
  if (codesResponse.status === "error") {
    console.error("Error extracting course codes; exiting.");
    process.exit(1);
  }

  // (B) SCRAPE COURSES FROM DEPARTMENTS
  // Gather all recognized mandatory & elective codes
  const allCodes = [
  ...
  // (C) FILTER RELEVANT SCRAPED COURSES ONLY
  // Normalize for matching similar to Python ("COMP 1405" → "COMP1405")
  const requestedCodesSet = new Set(allCodes.map(code => code.toUpperCase().replace(/\s/g, '')));
  const relevantCourses = [];
  allScrapedCourses.forEach(c => {
    const normalizedScraped = c.course_code.toUpperCase().replace(/\s/g, '');
    if (requestedCodesSet.has(normalizedScraped)) {
      relevantCourses.push(c);
    }
  });

  // (D) WRITE ONLY THE RELEVANT COURSES TO A MARKDOWN FILE
  const outputFile = "scraped_courses.md";
  // Generate markdown content similar to the Python version.
  ...
  console.log(`\nWrote ${relevantCourses.length} relevant courses to ${outputFile}`);

  // (E) GENERATE SCHEDULES
  ...
  console.log(JSON.stringify(schedulesResponse, null, 2));
}
