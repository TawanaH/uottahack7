...
def fetch_courses_for_department(department: str, term_code: str = "202510") -> List[ScheduleCourse]:
    ...
    while i < row_count:
        ...
        if len(tds) >= 11:
            ...
            if i_next < row_count:
                next_row = all_rows[i_next]
                next_text = next_row.get_text(separator=" ", strip=True)
                if "Meeting Date:" in next_text:
                    days_match = re.search(r"Days:\s*(.+?)\s*Time:", next_text)
                    time_match = re.search(r"Time:\s*(\d{1,2}:\d{2})\s*-\s*(\d{1,2}:\d{2})", next_text)
                    if days_match:
                        day_string = days_match.group(1).strip()
                    if time_match:
...
if __name__ == "__main__":
    ...
    codes_response = get_course_codes(
        ...
        audio_file_path=audio_file_path,  # Will use raw text since this is None.
