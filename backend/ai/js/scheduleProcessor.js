...
export async function getCourseCode(
  ...
  if (rawText && rawText.trim()) {
    ...
    try {
      // Use a read stream per Groq documentation:
...
/*
  ...
  It then returns a JSON response that is the final schedules output.
*/
export async function generateSchedulesFromInput({ rawText = "", audioFilePath = "" } = {}) {
  // Instantiate the Groq client and define allowed course codes.
  ...
  const codesResponse = await getCourseCode(client, allowedCodes, rawText, audioFilePath, "whisper-large-v3-turbo", "Specify context or spelling", "llama3-70b-8192");
  ...
  const schedulesResponse = await generateSchedules(client, outputFile, mandatoryList, electiveList, codesResponse.special_request, "llama3-70b-8192");
...
//   // (A) GET COURSE CODES (prefer raw text if available; otherwise use audio file)
//   // const audioFilePath = "test3.m4a"; // Replace with your audio file
//   const audioFilePath = ""
//   const rawTextInput = "I want to mandatory register COMP 1405, COMP 1805, MATH 1007, BIOL1902, PSYC1001"; // No raw text provided, so audio is used
...
//   // (C) WRITE ONLY THE RELEVANT COURSES TO A MARKDOWN FILE
//   const outputFile = "scraped_courses.md";
//   let mdContent = "# Course List\n\n";
//   relevantCourses.forEach(c => {
//     mdContent += `## ${c.course_code} (Section ${c.section})\n\n`;
